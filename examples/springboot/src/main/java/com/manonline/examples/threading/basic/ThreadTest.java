...
public class ThreadTest {
    /**
     * =========================================
     * sleep() : 在一个运行系统中，如果较高优先级的线程没有调用sleep方法，也没有受到I/O阻塞，那么较低优先级线程只能等待所有较高优先级的
     * 线程运行结束，方可有机会运行。sleep(long)使当前线程进入停滞状态，所以执行sleep()的线程在指定的时间内肯定不会被执行；
     ...
     * synchronized关键字用于保护共享数据，阻止其他线程对共享数据的存取，但是这样程序的流程就很不灵活了，如何才能在当前线程还没退出
     * synchronized数据块时让其他线程也有机会访问共享数据呢？
     * 此时就用这三个方法来灵活控制。
     * wait() : 方法使当前线程暂停执行并释放对象锁标示，让其他线程可以进入synchronized数据块，当前线程被放入对象等待池中。
     * notify() : 将从对象的等待池中移走一个任意的线程并放到锁标志等待池中，只有锁标志等待池中线程能够获取锁标志；如果锁标志等待池中
     * 没有线程，则notify()不起作用。
    ...
    private static void threadTesting1() {
        ...
        /**
         * 用户进程 v.s. 守护(服务)进程 setDameon(true);
         * -- OS (UNIX) : 开启进程。 守护进程。
         * -- JVM : 开启线程
         * http://blog.csdn.net/shimiso/article/details/8964414
         *
         * 守护线程程为了给用户线程提供服务，比方说GC, 如果没有了用户进程，守护进程也就没有必要存在。
         */
        // 第三种
        // 1、为什么没有打印出100句呢?因为我们将thread线程设置为了daemon(守护)线程,程序中只有守护线程存在的时候,是可以退出的,所以只打
        // 印了七句便退出了
        // 2、当java虚拟机中只有有守护线程在运行的时候，java虚拟机会关闭。当所有常规线程运行完毕以后，守护线程不管运行到哪里，虚拟机都会
        // 退出运行。所以你的守护线程最好不要写一些会影响程序的业务逻辑。否则无法预料程序到底会出现什么问题
